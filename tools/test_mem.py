...
def eval_single_ckpt_mem_meas(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=False, ckpt_path=None):
    # load checkpoint
    if ckpt_path is None:
        model.load_params_from_file(filename=args.ckpt, logger=logger, to_cpu=dist_test,
                                    pre_trained_path=args.pretrained_model)
    ...
    # start evaluation
    eval_utils.eval_one_epoch_mem_meas(
        cfg, args, model, test_loader, epoch_id, logger, dist_test=dist_test,
        result_dir=eval_output_dir
    )
...
def main():
    ...
    top_percentage = [1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1]
    # Note: When classes are added to backbone_3d.py they must be added to pcdet/models/backbones_3d/__init__.py aswell
    entr_classes = ['VoxelResBackBone8xEntropy1',
                    'VoxelResBackBone8xEntropy2',
                    'VoxelResBackBone8xEntropy3',
                    'VoxelResBackBone8xEntropy4']
    # top_percentage = [1.0]
    for i in top_percentage:
        for layer_num in entr_classes:
            cfg.MODEL.BACKBONE_3D.TOP_PERCENTAGE = i
            cfg.MODEL.BACKBONE_3D.NAME = layer_num
            model = build_network(model_cfg=cfg.MODEL, num_class=len(cfg.CLASS_NAMES), dataset=test_set)
            with torch.no_grad():
                # if args.eval_all:
                #     repeat_eval_ckpt(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=dist_test)
                # else:
                #     eval_single_ckpt(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=dist_test)
                print(f"Starting evaluation for top percentage {i} with Backbone3D class {layer_num}")
                eval_single_ckpt_mem_meas(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=dist_test)
                print(f"Finished evaluation for top percentage {i} with Backbone3D class {layer_num}")
